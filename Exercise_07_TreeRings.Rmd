...
```{r}
###QUESTION 1, MODIFIED ITERATIONS###
n.iter = 20000
...
```
(Original model with modified iterations noted above)
```
...
n.iter = 20000
  ...
  ## compile JAGS model
  jrand.model   <- jags.model (file = textConnection(RandomEffectsModel),
                           data = data,
                           inits = init,
                           n.chains = 3)
  ## burn-in
  jagsrand.out   <- coda.samples (model = jrand.model,
                              variable.names = c("tau_add","tau_dbh","tau_inc","mu","tau_ind","tau_yr"),
                              n.iter = min(n.iter,2000))
  plot(jagsrand.out)
...
```{r, fig.height=8}
  ...
  for(i in smp){
    sel = which(ci.names$row == i)
    plot(data$time,ci[2,sel],type='n',ylim=range(ci[,sel],na.rm=TRUE),ylab="DBH (cm)",main=i)
    ciEnvelope(data$time,ci[1,sel],ci[3,sel],col="green")
    points(data$time,data$z[i,],pch="+",cex=1.5)
  }
...
par(mfrow=c(1,1))
  ...
  if(length(year.cols>0)){
    ci.yr <- apply(out[,year.cols],2,quantile,c(0.025,0.5,0.975))
    plot(data$time,ci.yr[2,],type='n',ylim=range(ci.yr,na.rm=TRUE),main="Year Effect",ylab="cm")
    ciEnvelope(data$time,ci.yr[1,],ci.yr[3,],col="green")
    lines(data$time,ci.yr[2,],lty=1,lwd=2)
    abline(h=0,lty=2)
  }
...
```
The confidence intervals from the model including random effects are narrower than the original model's. 
```
...
```
Adding a random effect to DBH might help account for some measurement variability.
```
...
```
A violin plot of the original variables would help indicate which variables were causing the most variability, and could be compared to similar plots after adding effects to evaluate the reduction in unexplained variability.
```

5. Write the JAGS code that would fit the proposed model (note: you don't have to run this model, just propose the code)

```
Add these to the random effects model...
...
#### Process Model
  for(t in 2:nt){
    Dnew[i,t] <- x[i,t-1] + mu + ind[i] + year[t] + tau_dbh[i]
    x[i,t]~dnorm(Dnew[i,t],tau_add)
  }
...
#### DBH effects
  for(i in 1:ni){
    DBH[i] ~ dnorm(0,tau_DBH)
  }
  
  
```
```
Lab completed with Michael Silano
```
