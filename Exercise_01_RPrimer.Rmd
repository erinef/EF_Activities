...
3.	How is the atan2 function different from the atan function?
  ...
  atan is only for the 1st and 4th quadrants, atan2 is for all four.
  
4.	What is the difference between log and log10?

  log is for natural logs, and log10 is base ten logarithms.
...
8.	Describe the difference in output between sum and cumsum.
  ...
  "sum"" returns the total sum of every value in the vector. "cumsum"" returns a list of sums made by adding
  the current sum to the current value in the vector.
...
12.	Save the frog data file delimited in a pipe-delimited format (i.e. separated by ‘|’). Open the file up in a text editor and cut-and-paste the first few lines of the file into your Rscript file (just so I can see what you did).  Load the data back up into R as ‘dat2’ (show code)
  ...
  ```{r}
  write.table(dat,"my_frogs_piped.txt",row.names=FALSE,sep="|")
  ```
...
16.	Create a character vector that contains the names of 4 super heros.
  ...
  ```{r}
  vec = c("Ant Man", "Deadpool", "Batman", "Flash")
  ```
...
18.	For the frog data set: 
  ...
  d.	create a new object containing just the rows where there are between 3 and 5 tadpoles
    ...
    ```{r}
    newobj = dat[dat$tadpoles<=3 & dat$tadpoles>=5,]
    ```
...
23.	Using the frog data
  ...
  ```{r}
  fit = lm(tadpoles ~ (dat$color=="red"), data=dat)
  summary(fit)
  ```
  b.	Make a scatter plot of this data that includes the regression line
  
  ```{r}
  plot(dat$color=="red", dat$tadpoles)
  abline(fit)
  ```
  ...
  ```{r}
  fit2 = lm(tadpoles ~ frogs, data=dat)
  summary(fit2) # significant
  
  fit3 = lm(tadpoles ~ spots, data=dat)
  summary(fit3) # not significant
  
  fit4 = lm(tadpoles ~ color, data=dat)
  summary(fit4) # significant
  
  fit5 = lm(tadpoles ~ frogs + color, data=dat)
  summary(fit5) # only frogs significant
  
  fit6 = lm(tadpoles ~ frogs + spots + color, data=dat)
  summary(fit6) # only frogs significant
  ```
```
...
my.plot(subset(dat, dat$tadpoles > 5))
```
```
...
N = rep(N0,NT)
for(t in 2:NT){
	N[t] = N[t-1] + r*N[t-1]    ## discrete logistic growth
}
plot(N)
```
```
