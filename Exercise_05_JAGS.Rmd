...
```{r}
##Lab done with Michael Silano
initsN <- list() ##  near sample mean of 16.57
initsN[[1]] <- list(mu = 10)
initsN[[2]] <- list(mu = 15)
initsN[[3]] <- list(mu = 20)

N.model   <- jags.model (file = textConnection(NormalMeanN),
                             data = data,
                             inits = initsN,
                             n.chains = 3)


jags.out   <- coda.samples (model = N.model,
                            variable.names = c("mu"),
                                n.iter = 1000)
summary(jags.out) ## mean: 16.57
plot(jags.out) ## parameter density
BGR <- gelman.plot(jags.out) ##BGR = MCMC history
burnin <- BGR$last.iter[tail(which(BGR$shrink[,,2] > 1.1),1)+1] ##
## check for no burn-in case
if(length(burnin) == 0) burnin = 1
## remove burn-in
jags.burn <- window(jags.out,start=burnin)
...
gelman.diag(jags.burn)
plot(jags.burn)
##jags.out2 <- coda.samples(N.model,variable.names = c("mu"),10000)

## MCMC length: 1000 iterations 
##Total of 3 chains
burnin ##Burnin value
effectiveSize(jags.burn) ##Effective Sample Size
```
...
model {
  ...
  SD ~  dunif(0.1,500) ## precision prior on variance
  P <- 1/(SD*SD)

  for(i in 1:N){
    X[i] ~ dnorm(mu,P) # data model
  }
}
"
...
```
The prior is the uniform distribution (which is always positive, as required by a prior) of the min/max possible standard deviation of the data. 
```
...
```{r}
N2.model   <- jags.model (file = textConnection(NormalMeanN2),
                             data = data,
                             inits = initsN,
                             n.chains = 3)


jags.out   <- coda.samples (model = N2.model,
                            variable.names = c("mu"),
                                n.iter = 1000)
summary(jags.out) ## mean: 16.57, SD: .303
plot(jags.out) ## parameter density
BGR <- gelman.plot(jags.out) ##BGR = MCMC history
burnin <- BGR$last.iter[tail(which(BGR$shrink[,,2] > 1.1),1)+1] ##
## check for no burn-in case
