...
abund = tapply(fia$TREECOUNT,fia$SPCD,sum,na.rm=TRUE)
SPCDsort = sort(abund, decreasing=TRUE)
plot(SPCDsort, type='l', log='y', xlab="Rank", ylab="Abundance")
```
...
```{r}
eddy_html = getURL("http://fluxnet.ornl.gov/site_status")
eddy_table = readHTMLTable(eddy_html)
eddy_table

##code = rep(NA, 826)
##  for (i in 1:826){
##    code[i] = substr(eddy_table[i,2],1,2)
##  }
##codesort = sort(eddy_table$FlUXNET_ID, decreasing = TRUE)
...
wlef = nc_open("US-PFa-WLEF-TallTowerClean-2012-L0-vFeb2013 (4).nc")
print(wlef)    ## metadata
```
...
## Got this error when retrieving file. Running on Windows.
```
...
```
```
EVI is highest in the summer months, which is when CO2 fluxes would be most active and when atmospheric CO2 is lowest. 
```
