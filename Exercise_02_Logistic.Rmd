...
n = c(10,100,1000,10000)    # sequence of sample sizes
for(i in 1:4){          # loop over these sample sizes
  hist(rpois(n[i],1),main=n[i],probability=TRUE,breaks=40)  
                #here breaks defines number of bins in the histogram
  lines(x,dpois(x,1),col=2)
}
```
...
```{r}
z = dlnorm(x,meanlog=0,sdlog=0.5)
y = sin(x)

hist(z, main="Original Distribution", breaks=40)
abline(v=quantile(x,c(0.025,0.5,0.975)),lty=c(2,1,2),lwd=3,col="orange")
...
Unfortunately, R doesn't have a built in function for plotting shaded CI on time-series plots, so we'll define one. Unlike plot, which just takes x and y as arguements, this function needs to take time-series for both the upper (yhi) and lower (ylo) intervals.
```{r}
...
=======
  ylo[1])), border = NA,col=col,...) 
>>>>>>> 64dfe21ef290812b31cd5ed51d91476253c60073
}
```
...
```{r}
hist(n[,15]) ## ylim=c(0,200), xlim=c(5,15)
abline(v=mean(n[,15]), col="red")

## note: this histogram plots completely different graphs when the program is run vs when it is knitted, and adding the x and y lims did not fix the problem. The histogram it draws when it is run shows a bell curved set of bars between 5 and 14 with a mean of 10.
```

7. Plot the median trajectory and 95% CI.

```{r}
median(n) ## 9.503213 (row 757)
m = median(n)
s = sd(n)
hi = c(NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA)
lo = c(NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA)
for (i in 1:30) {
  hi[i] = m + (1.96 * s/sqrt(1000))
  lo[i] = m - (1.96 * s/sqrt(1000))
}

plot(n[757,], type='l')
ciEnvelope(n[757,],hi,lo,col="lightgrey")
